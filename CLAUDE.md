...
1. **`setup_coords.py`** — PDFをページ画像としてレンダリングし（`DISPLAY_SCALE=1.5`）、GUI上のクリックで座標を採取する。2フェーズ構成：まず各問の正答位置（`PHASE_QUESTIONS`）、全問終わると点数表示位置（`PHASE_SCORE`）。クリック座標は表示スケールで割ってPDF座標に変換して保存する。`questions`（`id`/`page`/`x`/`y`）と `score_*`、各種描画パラメータを `config.json` へ書き出す。
